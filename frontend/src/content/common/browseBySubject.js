                         ...
                         <Slider className="category_wrap" {...categoriesScrollSettings}>
                            {
                                ...
                                this.state.categorywise_contents && Object.keys(this.state.categorywise_contents).length > 0 ?
                                    this.state.categorywise_contents.map((category, index) => {
                                        let image = (category.category_image) ? category.category_image : funcObj.assets_path("/images/dummy-category.jpg");
                                        let active_tab = '';
                                        if (this.state.active_category == category.category_name) {
                                            active_tab = 'active';
                                        }
                                        return (
                                            <div className="text-center" key={index}>
                                            <img className='cate_icon' src={image}  onClick={(e) =>this.showTabs(e,category)} />
                                                <Link to="" className={`nav-link category-link px-3 py-2 py-lg-5 text-center  border-0 ` + active_tab}  onClick={(e) =>this.showTabs(e,category)}>
                                                
                                                {category.category_title_p}</Link>
                                            </div>
                                        )
                                    })
                                    : null
                    ...
                    <div className="tab-content" id="myTabContent">
...

                        {
                            this.state.categorywise_contents && Object.keys(this.state.categorywise_contents).length > 0 ?
                                this.state.categorywise_contents.map((category, index) => {
                                    let default_active = '';
                                    if (category.category_name.toLowerCase() == this.state.active_category.toLowerCase()) {
                                        default_active = 'active show';
                                    }
        
                                    return (
                                        this.looping_books_tab(category.contents, category, index)
                                    )
                                })
                                : null
                        }
                        ...
                        <div className="text-center">
                            <button className="btn signinBtn sign_in_btn" onClick={(e) => funcObj.openUrl(e, this.props.view_all_link)}> View All &nbsp; <i className="fas fa-arrow-right"></i></button>
                        </div>
                    </div>
            </>
        )
                    ...
                    <div className="books-cover row mt-5 px-3 px-lg-5 top-categories">
                        {
                            ...
                            all_books.data.map((content, index) => {
                                    ...
                                    return (
                                        <div key={index} className={`col-xl-${this.props.bcols} col-md-${this.props.bcols} mb-3 mb-lg-5`}>
                                            <div className="book-wrap-card">
                                                <div className="card-book">
                                                    ...
                                                    <div className="book-details">
                                                        ...
                                                        <div className="book-name mt-0 my-2">
                                                            {content.title}
                                                        </div>
